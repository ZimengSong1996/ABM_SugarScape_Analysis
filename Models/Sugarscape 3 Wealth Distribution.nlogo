...
to turtle-setup ;; turtle procedure
  ...
  set age 0
  set vision random-in-range 25 30
...
to go
  ...
  update-lorenz-and-gini
  let newgen top-20-percent-new-generation
  print (word "财富前20%新生代比例: " newgen)
  tick
end
...
;; add report
;;
to-report calculate-gini-index
  let num-people count turtles
  let sorted-wealths sort [sugar] of turtles
  let total-wealth sum sorted-wealths
  let wealth-sum-so-far 0
  ...
  let gini (gini-index-reserve / count turtles) * 2
  report gini
end

to-report top-20-percent-new-generation
  let threshold-age 30 ; define the threshold of young people
  let sorted-turtles sort-on [sugar] turtles ; sort turtles according to sugar
  let top-20-percent-count floor (0.2 * length sorted-turtles) ; caculate the number of top 20% rich group
  let top-20-percent-turtles sublist sorted-turtles 0 top-20-percent-count ; get the list of top 20% rich group
  let new-generation-turtles filter [a-turtle -> [age] of a-turtle < threshold-age] top-20-percent-turtles ; filter to get the young agents
  let new-generation-agentset turtle-set new-generation-turtles ; transform young agents list to agentset
  let new-generation-count count new-generation-agentset ; calculate the number of young agents
  ; calculate and retrun the young proportion in top 20% rich group
  report new-generation-count / top-20-percent-count
end

;to-report death-rate-due-to-resource-depletion
;  let dead-turtles count turtles with [sugar <= 0]
;  let total-turtles count turtles
;  if total-turtles = 0 [report 0]
;  report (dead-turtles / total-turtles) * 100
;end
...
"" ""
PENS
"default" 1.0 0 -16777216 true "" "plotxy ticks sum [sugar] of turtles"
...
210
395
top-wealth-new-generation-percent
NIL
NIL
0.0
100.0
...
"" ""
PENS
"default" 1.0 0 -16777216 true "" "plot top-20-percent-new-generation"
...
210
555
death-rate-due-to-resource-depletion
NIL
NIL
0.0
100.0
...
"" ""
PENS
"default" 1.0 0 -16777216 true "" "plot death-rate-due-to-resource-depletion"
...
@#$#@#$#@
<experiments>
  <experiment name="initial-wealth" repetitions="5" runMetricsEveryStep="true">
    <setup>setup</setup>
    <go>go</go>
    <timeLimit steps="500"/>
    <metric>top-20-percent-new-generation</metric>
    <metric>calculate-gini-index</metric>
  ...
  <experiment name="metabolism" repetitions="5" runMetricsEveryStep="true">
    <setup>setup</setup>
    <go>go</go>
    <timeLimit steps="500"/>
    <metric>calculate-gini-index</metric>
  ...
  <experiment name="vision" repetitions="5" runMetricsEveryStep="true">
    <setup>setup</setup>
    <go>go</go>
    <timeLimit steps="500"/>
    <metric>calculate-gini-index</metric>
